---
title: 'Final Project'
author: "by Jana Jaber and Hadi Bazzi"
...
library(caret)
library(e1071)
df_encoded$diabetes <- factor(df_encoded$diabetes, levels = unique(df_encoded$diabetes),
                              labels = c("0", "1"))

train_y_encoded <- factor(train_y_encoded, levels = c("0", "1"), 
                          labels = c("Class0", "Class1"))
test_y_encoded <- factor(test_y_encoded, levels = c("0", "1"), 
                         labels = c("Class0", "Class1"))
...
2. **Similarity Measurement**: Calculate distances between all pairs of clusters using a chosen metric (like Euclidean distance).

3. **Cluster Merging**: Repeatedly merge the closest pair of clusters.

4. **Hierarchy Formation**: Each merge forms part of a hierarchy represented as a tree structure (dendrogram).

5. **Termination**: The process continues until all data points are merged into a single cluster.

6. **Number of Clusters**: Decide the number of clusters by cutting the dendrogram at a desired level.
